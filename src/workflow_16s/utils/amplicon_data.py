...
class StatisticalAnalyzer:
    ...
    def get_effect_size(
        self, test_name: str, result_row: pd.Series
    ) -> Optional[float]:
        """
...
class Ordination:
    ...
    def __init__(
        self, cfg: Dict, output_dir: Union[str, Path], verbose: bool = False
    ):
        self.cfg = cfg
        self.verbose = verbose
    ...
    def run_tests(
        ...
        **kwargs,
        ...
        try:
            ...
            if self.verbose:
                logger.info(
                    f"Aligned table: {table.shape[0]} features × {table.shape[1]} samples"
                )
            ...
            for test_name in tests_to_run:
                ...
                try:
                        ...
                        plot_kwargs.update(
                            {
                                ...
                                "proportion_explained": ordination_result[
                                    "exp_var_ratio"
                                ],
                            }
                        )
                    ...
                    if self.verbose:
                        logger.error(
                            f"Failed {test_name} for {transformation}: {str(e)}"
                        )
            ...
            if progress and task_id:
                progress.update(
                    main_task, advance=total_tests_to_run
                )  # Use filtered count
...
def filter_and_reorder_biom_and_metadata(
    ...
    metadata_df[sample_column] = (
        metadata_df[sample_column].astype(str).str.lower()
    )
    ...
    metadata_df = (
        metadata_df.set_index(sample_column).loc[shared_ids].reset_index()
    )

    # Map back to original-case sample IDs for biom
    ordered_biom_sample_ids = [
        lowercase_to_original[sid] for sid in metadata_df[sample_column]
    ]
    ...
    sample_index = {
        sid: i for i, sid in enumerate(table_filtered.ids(axis="sample"))
    }
    reordered_indices = [
        sample_index[sid] for sid in ordered_biom_sample_ids]
...
class AmpliconData:
    ...
    def __init__(self, cfg, project_dir, mode="genus", verbose=False):
        ...
        self.ml_results = {}  # Store ML results for multiple methods/levels
    ...
    def _load_metadata(self):
        ...
        self.meta = import_merged_meta_tsv(meta_paths, None, self.verbose)
    ...
    def _filter_and_align_data(self):
        ...
        if self.cfg["figures"]["map"]:
            self.plotter = Plotter(
                self.cfg, self.figure_output_dir, self.verbose)
    ...
    def _apply_preprocessing(self):
        ...
        normalization_enabled = (
            self.cfg["features"]["filter"] and self.cfg["features"]["normalize"]
        )
        clr_transformation_enabled = (
            self.cfg["features"]["filter"]
            and self.cfg["features"]["normalize"]
            and self.cfg["features"]["clr_transform"]
        )
        enabled_steps = [
            filtering_enabled,
            normalization_enabled,
        ...
        with create_progress() as progress:
            main_task = progress.add_task(
                f"[white]Preprocessing {self.mode} tables".ljust(
    ...
    def _save_tables(self):
        ...
        total_tables = sum(
            len(level_tables) for level_tables in self.tables.values()
        )
    ...
    def _run_ordination(self):
        ...
        with create_progress() as progress:
            main_task = (
                progress.add_task(
                    "[white]Running ordination analyses".ljust(
                        DEFAULT_PROGRESS_TEXT_N
                    ),
                    total=total_plots,
                )
            ...
            for table_type, level_tables in self.tables.items():
                ...
                for level, table in level_tables.items():
                    ...
                    ordination_output_dir = (
                        self.figure_output_dir / level / table_type
                    )
                    ...
                    if self.verbose:
                        logger.info(
                            f"Stored ordination results for {table_type}/{level}:"
                        )
    ...
    def _identify_top_features(self):
        ...
        (
            self.top_contaminated_features,
            self.top_pristine_features,
        ) = analyzer.analyze(self.stats, DEFAULT_GROUP_COLUMN)
    ...
    def _run_ml_feature_selection(self):
        ...
        # Get lists of methods and levels to try
        methods = ml_cfg.get("methods", ["catboost"])
        table_types = ml_cfg.get("table_types", ["clr_transformed"])
        levels = ml_cfg.get("levels", ["genus"])
        num_features = ml_cfg.get("num_features", 100)

        total_combinations = len(methods) * len(table_types) * len(levels)
        if total_combinations == 0:
            logger.warning("No ML configurations specified. Skipping ML.")
            return

        # Initialize ML results storage
        self.ml_results = {}

        with create_progress() as progress:
            ml_task = progress.add_task(
                "[white]Running ML feature selection".ljust(
                    DEFAULT_PROGRESS_TEXT_N
                ),
            ...
            for method in methods:
                for tbl_type in table_types:
                    for level in levels:
                        # Skip if table type/level not available
                        if (
                            tbl_type not in self.tables
                            or level not in self.tables[tbl_type]
                        ):
                            logger.warning(
                                f"Skipping ML: {tbl_type}/{level} not available"
                            )
                        ...
                        y = self.meta.set_index("#sampleid")[
                        ...
                        combo_id = f"{method}/{tbl_type}/{level}"
                        if self.verbose:
                            logger.info(
                                f"Running ML ({combo_id}) with {X.shape[1]} features"
                            )
                        ...
                        try:
                            ...
                            X_tr, X_te, y_tr, y_te = filter_data(
                                X,
                                y,
                                self.meta.set_index("#sampleid").loc[
                            ...
                            X_tr_sel, X_te_sel, selected = perform_feature_selection(
                                ...
                                random_state=42,
                                use_permutation_importance=ml_cfg[
                            ...
                            logger.error(
                                f"ML failed for {combo_id}: {str(e)}"
                            )
                            if self.verbose:
                                logger.debug("Traceback:", exc_info=True)
                        finally:
