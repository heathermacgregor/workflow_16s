...
def generate_html_report(
    ...
    <head>
        ...
        <style>
            ...
            /* NEW: Tab styles for sample maps */
            .tabs {{
                display: flex;
                margin-bottom: -1px;
            }}
        ...
        {plotly_js_tag}
        <script>
            // Existing function for plot dropdowns
            ...
            // NEW: Functions for sample map tabs
            function showMapTab(tabId, plotId) {{
                // Hide all tabs
                ...
                if (tab) {{
                    ...
                    if (!window.mapInitializedPlots.has(plotId)) {{
                        renderMapPlot(`container-${{plotId}}`, plotId);
                        window.mapInitializedPlots.add(plotId);
                    }}
                }}
            ...
            function renderMapPlot(containerId, plotId) {{
                ...
                plotDiv.id = plotId;
                plotDiv.className = 'map-container';
                container.appendChild(plotDiv);
                
                // Get plot data
                const data = window.mapPlotData[plotId];
                if (!data) {{
    ...
    <body>
        ...
        <script>
            // Initialize map plot tracking
            window.mapInitializedPlots = new Set();
        </script>
    </body>
    </html>
    """
...
def _prepare_figures(figures: Dict) -> str:
    ...
    # NEW: Sample Maps with tabs
    if "map" in figures and figures["map"]:
        sample_maps = [(c, f) for c, f in figures["map"].items() if f][:2]
        if sample_maps:
            tabs = []
            buttons = []
            plot_data = {}
            
            for i, (col, fig) in enumerate(sample_maps):
                tab_id = f"map-tab-{i}"
                plot_id = f"map-plot-{i}"
                
                # Add tab button
                active = "active" if i == 0 else ""
                buttons.append(
                    f'<button class="tab-button {active}" data-tab="{tab_id}" '
                    f'onclick="showMapTab(\'{tab_id}\', \'{plot_id}\')">{col}</button>'
                )
                ...
                tabs.append(
                    ...
                    f'<div id="container-{plot_id}" class="map-container"></div>'
                    f'</div>'
                )
            ...
            <div class="figure-container">
                ...
                <script>
                    // Store plot data for sample maps
                    window.mapPlotData = {plot_data_json};
                    
                    // Initialize first tab
                    document.addEventListener('DOMContentLoaded', () => {{
                        const firstTab = document.querySelector('.tab-pane');
                        if (firstTab) {{
                            const plotId = firstTab.querySelector('.map-container').id.replace('container-', '');
                            showMapTab(firstTab.id, plotId);
                        }}
                    }});
                </script>
            </div>
